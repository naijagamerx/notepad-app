...
class FileOperations {
    ...
    setupFileUpload() {
        ...
        // Get references, might return null if elements don't exist
        const uploadBtn = document.getElementById('uploadBtn');
        const uploadTextBtn = document.getElementById('uploadTextBtn');

        // Check if fileInput exists before proceeding
        if (!fileInput) {
            console.error('File input element (#uploadInput) not found!');
            return; // Stop setup if input is missing
        }

        // Safely add listener to uploadBtn if it exists
        if (uploadBtn) {
            uploadBtn.addEventListener('click', () => fileInput.click());
        } else {
            // It's okay if this one is missing, might be conditional UI
            // console.warn('Upload button (#uploadBtn) not found.');
        }

        // Safely add listener to uploadTextBtn if it exists (this is the one causing the error)
        if (uploadTextBtn) {
            uploadTextBtn.addEventListener('click', () => fileInput.click());
        } else {
            // This might indicate an issue if it's expected to always be there
            console.warn('Upload Text button (#uploadTextBtn) not found.');
        }

        // Handle file selection (only if fileInput was found)
